...
- `path_reliability.voacap_fallback.enabled: false` starts no SSN polling or VOACAP worker. When enabled, it fetches NOAA SSN on the configured cadence, uses rounded integer 8-hour EWMA SSN generations, delays fallback after insufficient bucket evidence, and can only replace insufficient results when VOACAP is closed or when sparse bucket p50 and the cached current-hour VOACAP forecast map to the same path class. Existing sufficient bucket p50 results remain authoritative.
