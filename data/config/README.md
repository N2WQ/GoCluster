...
- `path_reliability.yaml` - operator policy for enable/display/sample-floor/receiver-cap mode/glyphs and the disabled VOACAP closed fallback, plus active p50 histogram compatibility and algorithm calibration for decay, weights, thresholds, offsets, and noise tables.
...
- `path_reliability.voacap_fallback.enabled: false` starts no SSN polling or VOACAP worker. When enabled, it fetches NOAA SSN on the configured cadence, uses rounded integer 8-hour EWMA SSN generations, delays fallback after insufficient bucket evidence, and can only emit the configured closed glyph when VOACAP FT8-equivalent SNR is at or below `mode_thresholds.<mode>.closed`. Existing bucket p50 results remain authoritative.
