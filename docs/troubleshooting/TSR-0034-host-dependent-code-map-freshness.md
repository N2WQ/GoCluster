...
- Why: `cmd/codemap` consumed host-selected `go list -json` file and import
  metadata. The maps were generated on Windows, while CI ran on Linux with CGO
  enabled and selected different build-tagged implementations and dependencies.
...
- How we know: an isolated Linux/amd64, CGO-enabled reproduction produced the
  ...
  regression fixtures now compare divergent host metadata against one expected
  inventory, dependency set, rendering, and fingerprint. Hosted confirmation
  remains pending until the change is pushed.
...
- Request reference (chat/issue/link): GitHub Actions run `29190757076`.

## Symptoms and Impact

- What failed or looked wrong? Ubuntu CI reported
  `runtime-ingest-fanout.md` and `path-reliability-voacap.md` as stale, while
  Windows reported `code maps are fresh` at the same commit.
...
- Root cause (or best current explanation): host-selected `GoFiles` and
  `Imports` were used as inputs to an artifact required to be identical across
  Windows development and Linux CI.
- Contributing factors: tests covered subprocess stream separation but did not
  present divergent platform metadata or assert platform-independent files,
  dependency edges, rendering, and fingerprints.
...
- Decision delta summary: replace host-selected package files and imports with
  full build-tagged source coverage.
