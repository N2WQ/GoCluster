...
func TestParsePredictionTotalsWithAndWithoutStale(t *testing.T) {
	withStale := "2026/04/20 12:00:00 Path predictions (5m): total=1,200 derived=5 combined=700 insufficient=500 no_sample=300 low_count=75 low_receiver=25 low_weight=50 stale=50 cap_limited=25 cap_would_block=10 override_r=0 override_g=0"
	got, ok := parsePredictionTotals(withStale)
	if !ok {
		t.Fatalf("expected prediction totals to parse")
	}
	if got.Total != 1200 || got.Combined != 700 || got.Insufficient != 500 || got.NoSample != 300 || got.LowCount != 75 || got.LowReceiver != 25 || got.LowWeight != 50 || got.Stale != 50 || got.CapLimited != 25 || got.CapWouldBlock != 10 {
...
func FuzzParsePathPredictionLogTotals(f *testing.F) {
	f.Add("2026/04/20 12:00:00 Path predictions (5m): total=1,200 derived=5 combined=700 insufficient=500 no_sample=300 low_count=75 low_receiver=25 low_weight=50 stale=50 cap_limited=25 cap_would_block=10 override_r=0 override_g=0")
...
func TestPredictionActivitySummarySeparatesCountAndWeightLimits(t *testing.T) {
	got := predictionActivitySummary([]predictionHour{
		...
		{Hour: "02:00", AvgTotal: 200, AvgCombined: 150, AvgInsufficient: 50, AvgLowCount: 5, AvgLowReceiver: 10, AvgLowWeight: 40},
	})
