...
`
	...
	if cfg.CallCorrection.StabilizerDelaySeconds != 15 {
		t.Fatalf("expected stabilizer delay 15s from shipped YAML, got %d", cfg.CallCorrection.StabilizerDelaySeconds)
	}
	if cfg.CallCorrection.StabilizerMaxChecks != 5 {
		t.Fatalf("expected stabilizer max checks 5 from shipped YAML, got %d", cfg.CallCorrection.StabilizerMaxChecks)
	}
	...
	if cfg.CallCorrection.StabilizerPDelayConfidencePercent != 25 {
		t.Fatalf("expected stabilizer P-delay confidence 25 from shipped YAML, got %d", cfg.CallCorrection.StabilizerPDelayConfidencePercent)
	}
	if cfg.CallCorrection.StabilizerPDelayMaxChecks != 3 {
		t.Fatalf("expected stabilizer P-delay max checks 3 from shipped YAML, got %d", cfg.CallCorrection.StabilizerPDelayMaxChecks)
	}
	...
	if !cfg.CallCorrection.StabilizerEditNeighborEnabled {
		t.Fatalf("expected stabilizer edit-neighbor delay enabled from shipped YAML")
	}
	...
	if cfg.CallCorrection.StabilizerEditNeighborMinSpotters != 2 {
		t.Fatalf("expected stabilizer edit-neighbor min spotters 2 from shipped YAML, got %d", cfg.CallCorrection.StabilizerEditNeighborMinSpotters)
	}
	if !cfg.CallCorrection.ResolverNeighborhoodEnabled {
		t.Fatalf("expected resolver neighborhood enabled from shipped YAML")
	}
	if cfg.CallCorrection.ResolverNeighborhoodBucketRadius != 1 {
		t.Fatalf("expected resolver neighborhood radius 1 from shipped YAML, got %d", cfg.CallCorrection.ResolverNeighborhoodBucketRadius)
	}
	...
	if cfg.CallCorrection.ResolverRecentPlus1MinUniqueWinner != 4 {
		t.Fatalf("expected resolver recent plus1 min unique winner normalized to custom SCP minimum 4, got %d", cfg.CallCorrection.ResolverRecentPlus1MinUniqueWinner)
	}
	...
	if !cfg.CallCorrection.BayesBonus.Enabled {
		t.Fatalf("expected bayes bonus enabled from shipped YAML")
	}
	...
	if !cfg.CallCorrection.TemporalDecoder.Enabled {
		t.Fatalf("expected temporal decoder enabled from shipped YAML")
	}
	...
	if cfg.CallCorrection.TemporalDecoder.LagSeconds != 15 {
		t.Fatalf("expected temporal decoder lag 15 from shipped YAML, got %d", cfg.CallCorrection.TemporalDecoder.LagSeconds)
	}
	if cfg.CallCorrection.TemporalDecoder.MaxWaitSeconds != 15 {
		t.Fatalf("expected temporal decoder max wait 15 from shipped YAML, got %d", cfg.CallCorrection.TemporalDecoder.MaxWaitSeconds)
	}
	...
	if cfg.CallCorrection.FamilyPolicy.Truncation.MaxLengthDelta != 2 {
		t.Fatalf("expected truncation max length delta 2 from shipped YAML, got %d", cfg.CallCorrection.FamilyPolicy.Truncation.MaxLengthDelta)
	}
	...
	if !cfg.CallCorrection.FamilyPolicy.Truncation.LengthBonus.Enabled {
		t.Fatalf("expected truncation length bonus enabled from shipped YAML")
	}
	if cfg.CallCorrection.FamilyPolicy.Truncation.LengthBonus.Max != 2 {
		t.Fatalf("expected truncation length bonus max 2 from shipped YAML, got %d", cfg.CallCorrection.FamilyPolicy.Truncation.LengthBonus.Max)
	}
	if !cfg.CallCorrection.FamilyPolicy.Truncation.Delta2Rails.Enabled {
		t.Fatalf("expected truncation delta2 rails enabled from shipped YAML")
	}
	if cfg.CallCorrection.FamilyPolicy.Truncation.Delta2Rails.ExtraConfidencePercent != 5 {
		t.Fatalf("expected truncation delta2 extra confidence 5 from shipped YAML, got %d", cfg.CallCorrection.FamilyPolicy.Truncation.Delta2Rails.ExtraConfidencePercent)
	}
	...
	if !cfg.CallCorrection.FamilyPolicy.TelnetSuppression.EditNeighborEnabled {
		t.Fatalf("expected telnet edit-neighbor suppression enabled from shipped YAML")
	}
	if cfg.CallCorrection.FamilyPolicy.TelnetSuppression.WindowSeconds != 180 {
		t.Fatalf("expected telnet family suppression window 180 from shipped YAML, got %d", cfg.CallCorrection.FamilyPolicy.TelnetSuppression.WindowSeconds)
	}
	...
	if cfg.CallCorrection.FamilyPolicy.TelnetSuppression.FrequencyToleranceFallbackHz != 1000 {
		t.Fatalf("expected telnet family suppression fallback tolerance 1000, got %.1f", cfg.CallCorrection.FamilyPolicy.TelnetSuppression.FrequencyToleranceFallbackHz)
	}
...
`,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			dir := testConfigDir(t)
			writeRequiredFloodControlFile(t, dir)
			writeTestConfigOverlay(t, dir, "pipeline.yaml", tc.body)
			_, err := Load(dir)
			if err == nil {
...
`
	...
	_, err := Load(dir)
	if err == nil {
		t.Fatalf("expected Load() error for invalid negative stabilizer/edit-neighbor settings")
	}
	if !strings.Contains(err.Error(), "stabilizer_edit_neighbor_min_spotters") {
		t.Fatalf("expected stabilizer_edit_neighbor_min_spotters error, got %v", err)
	}
}
