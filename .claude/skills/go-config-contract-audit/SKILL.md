...
2. Identify the single source of truth.
   ...
   - Runtime code must not fall back to `DefaultConfig()` for YAML-owned
     settings.
   - Runtime code must not blindly merge unrelated YAML files.
   - Any package-local loader bypass must be explicitly documented and
     approved.
...
3. Audit defaults and presence.
   ...
   - Classify each hit as validation constant, algorithm constant, test
     helper, compatibility boundary, or illegal runtime default.
...
6. Validate optional secret/tool config.
   - Keep optional config optional at server startup unless required by
