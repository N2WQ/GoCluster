...
var requiredConfigPaths = []yamlconfig.Path{
	...
	{"voacap_fallback", "forecast_cache_db_path"},
...
type VOACAPFallbackConfig struct {
	...
	ForecastCacheDBPath                    string    `yaml:"forecast_cache_db_path"`
...
func defaultVOACAPFallbackConfig() VOACAPFallbackConfig {
	return VOACAPFallbackConfig{
		...
		ForecastCacheDBPath:                    "data/voacap/forecast_cache.pebble",
...
func (c *VOACAPFallbackConfig) finalize() error {
	...
	c.ForecastCacheDBPath = strings.TrimSpace(c.ForecastCacheDBPath)
	if c.Enabled && c.ForecastCacheDBPath == "" {
		return fmt.Errorf("voacap_fallback.forecast_cache_db_path must not be empty when enabled")
	}
